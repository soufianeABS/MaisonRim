...
/** Same behavior as @supabase/ssr combineChunks — inlined so Edge bundles avoid `.../dist/module/utils/*` imports. */
...
/**
 * Edge-safe session check: no @supabase/supabase-js (avoids Node-only APIs in the Edge bundle).
 * Reads the same cookies as @supabase/ssr createServerClient and validates via Auth HTTP API.
 */
